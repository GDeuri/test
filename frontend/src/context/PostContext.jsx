...
export const PostContextProvider = ({ children }) => {
  ...
  async function addCommentReply(id, commentId, replyText) {
    try {
      const { data } = await axios.post("/api/post/reply", {
        postId: id,
        commentId: commentId,
        replyText,
      });
  ...
  async function deleteReply(postId, commentId, replyId) {
    try {
      const { data } = await axios.delete(
        `/api/post/reply/${postId}?commentId=${commentId}&replyId=${replyId}`
      );
