...
export const getOnePost = TryCatch(async (req, res) => {
  const post = await Post.findOne({ _id: req.params.id }).populate("owner", "-password").populate({
    path: "comments.user",
    select: "-password",
  });
...
export const replyOnComment = TryCatch(async (req, res) => {
